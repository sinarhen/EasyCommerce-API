...
public class BillboardRepository : BaseRepository, IBillboardRepository
{
    ...
    public async Task DeleteBillboardAsync(Guid collectionId, Guid billboardId, string userId, bool isAdmin)
    {
        ...
        if (!isOwner) throw new UnauthorizedAccessException("You are not authorized to delete this billboard");
    ...
    public async Task<Models.Entities.Billboard> UpdateBillboardAsync(Guid billboardId,
        UpdateBillboardDto updateBillboardDto, string userId, bool isAdmin)
    {
        var billboard = await _db.Billboards
        ...
        if (!isOwner) throw new UnauthorizedAccessException("You are not authorized to update this billboard");
        ...
        billboard.CollectionId = updateBillboardDto.CollectionId != Guid.Empty
            ? updateBillboardDto.CollectionId
            : billboard.CollectionId;
